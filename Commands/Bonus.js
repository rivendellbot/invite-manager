...
exports.run = async (client, message, args) => {
    if(!message.member.hasPermission("ADMINISTRATOR") && !message.member.hasPermission("MANAGE_GUILD")) return message.reply("Vous n'avez pas la permission");

    var victim = message.mentions.members.size > 0 ? message.mentions.members.first().id : args.length > 0 ? args[0] : undefined;
    if(!victim) return message.reply("Veuillez mentionner un membre !");
    victim = message.guild.member(victim);
    if(!victim) return message.reply("Le membre mentionné doit être dans le serveur.");

    var num = Number(args[1]);
    if(isNaN(num)) return message.reply("Vous devez mettre un nombre valide.");
